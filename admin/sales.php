            ...
            <div class="box-header with-border">
              <div class="pull-right">
                <form method="POST" class="form-inline" action="sales_print.php">
                  ...
                  <button type="button" class="btn btn-primary btn-sm btn-flat" id="exportButton"><i class="fa fa-download"></i> Export to Excel</button>
                </form>
...
$(function(){
  ...
  $('#exportButton').click(function() {
    // Clone the table
    var table = $('#example1').clone();

    // Remove the last column from the cloned table
    table.find('tbody tr').each(function() {
      $(this).find('td:last-child').remove();
    });
    table.find('thead th:last-child').remove();

    // Remove any unwanted elements from the cloned table (e.g., hidden columns, action buttons)
    table.find('.hidden').remove();
    table.find('.btn').remove();

    // Convert the table to Excel format using the SheetJS library
    var wb = XLSX.utils.table_to_book(table[0], { sheet: "Sheet 1" });
